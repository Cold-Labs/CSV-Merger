        ...
        try:
            ...
            for field, alternatives in company_mappings.items():
                if not isinstance(alternatives, list):
                    ...
                    warnings.append(
                        f"Company mapping for '{field}' has many alternatives ({len(alternatives)})"
                    )
            ...
            for field, alternatives in people_mappings.items():
                if not isinstance(alternatives, list):
                    ...
                    warnings.append(
                        f"People mapping for '{field}' has many alternatives ({len(alternatives)})"
                    )
            ...
            for field in all_fields:
                if not field.strip():
                    ...
                    warnings.append(
                        f"Field name '{field}' is very long ({len(field)} characters)"
                    )
    ...
    def list_backups(self) -> list:
        ...
        try:
            ...
            for filename in os.listdir(self.backup_dir):
                if filename.startswith("field_mappings_backup_") and filename.endswith(
                    ".json"
                ):
