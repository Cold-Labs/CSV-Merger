...
class Phase3Enricher:
    ...
    def _load_personal_email_domains(self) -> Dict[str, int]:
        ...
        try:
            ...
            personal_domains = field_mappings.get("data_cleaning_rules", {}).get(
            ...
            logger.warning(
                f"Failed to load personal domains from config: {e}, using fallback"
            )
            # Fallback to hardcoded list
            return {"gmail.com": 4, "yahoo.com": 3, "outlook.com": 2, "hotmail.com": 1}
    ...
    def enrich_and_deduplicate(
        self, standardized_df: pd.DataFrame, table_type: str
    ) -> pd.DataFrame:
        """
        ...
        logger.info(
            f"   • Personal emails ranked: {self.stats['personal_emails_ranked']}"
        )
        logger.info(
            f"   • Work emails identified: {self.stats['work_emails_identified']}"
        )
        logger.info(f"   • Names split: {self.stats['names_split']}")
        logger.info(
            f"   • Company names generated: {self.stats['company_names_generated']}"
        )
    ...
    def _enrich_company_domain(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Company Domain" not in df.columns or "Work Email" not in df.columns:
            logger.warning(
                "Company Domain or Work Email column not found, skipping domain enrichment"
            )
            return df
        ...
        for idx in df.index:
            ...
            if (
                pd.isna(company_domain)
                or not str(company_domain).strip()
                or str(company_domain).lower() in ["nan", "none", "null", ""]
            ) and (
                not pd.isna(work_email)
                and str(work_email).strip()
                and "@" in str(work_email)
                and str(work_email).lower() not in ["nan", "none", "null", ""]
            ):
                ...
                try:
                    ...
                    if email_domain not in free_providers:
                        ...
                        enriched_count += 1
                        logger.debug(
                            f"Enriched Company Domain from Work Email: {work_email} -> {email_domain}"
                        )

                except (IndexError, AttributeError) as e:
                    # Invalid email format, skip
                    logger.debug(
                        f"Invalid email format for domain extraction: {work_email}"
                    )
                    continue
    ...
    def _split_full_names(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in df.index:
            ...
            if (
                not pd.isna(full_name)
                and str(full_name).strip()
                and str(full_name).lower() not in ["nan", "none", "null", ""]
            ) and (
                (
                    pd.isna(first_name)
                    or not str(first_name).strip()
                    or str(first_name).lower() in ["nan", "none", "null", ""]
                )
                or (
                    pd.isna(last_name)
                    or not str(last_name).strip()
                    or str(last_name).lower() in ["nan", "none", "null", ""]
                )
            ):
                ...
                try:
                    ...
                    if len(name_parts) >= 2:
                        ...
                        split_count += 1
                        logger.debug(
                            f"Split name: '{full_name}' -> First: '{name_parts[0]}', Last: '{name_parts[-1]}'"
                        )
    ...
    def _enrich_company_name(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Company Domain" not in df.columns:
            logger.warning(
                "Company Domain column not found, skipping company name generation"
            )
            return df
        ...
        for idx in df.index:
            company_name = (
                df.at[idx, "Company Name"] if "Company Name" in df.columns else ""
            )
            company_domain = df.at[idx, "Company Domain"]

            # If Company Name is missing but Company Domain exists
            if (
                pd.isna(company_name)
                or not str(company_name).strip()
                or str(company_name).lower() in ["nan", "none", "null", ""]
            ) and (
                not pd.isna(company_domain)
                and str(company_domain).strip()
                and str(company_domain).lower() not in ["nan", "none", "null", ""]
            ):
                ...
                try:
                    ...
                    if domain_name:
                        ...
                        if "-" in domain_name:
                            company_name_generated = " ".join(
                                word.capitalize() for word in domain_name.split("-")
                            )
                        else:
                        ...
                        generated_count += 1
                        logger.debug(
                            f"Generated company name: '{domain}' -> '{company_name_generated}'"
                        )

                except Exception as e:
                    logger.debug(
                        f"Error generating company name from domain '{company_domain}': {e}"
                    )
                    continue
    ...
    def _clean_domains(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        domain_columns = [
            ...
            if "domain" in col.lower() or "website" in col.lower()
        ]
    ...
    def _normalize_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        name_columns = [
            col
            for col in df.columns
            if any(word in col.lower() for word in ["name", "title"])
        ]
        for col in name_columns:
            if col in df.columns:
                df[col] = df[col].apply(
                    lambda x: str(x).title() if x and str(x).lower() != "nan" else ""
                )
        ...
        if cleaned_integer_count > 0:
            logger.info(
                f"Cleaned {cleaned_integer_count} integer field values (removed decimals)"
            )
    ...
    def _smart_deduplicate(self, df: pd.DataFrame, table_type: str) -> pd.DataFrame:
        ...
        for idx in df.index:
            ...
            for key_col in available_keys:
                ...
                if (
                    value
                    and str(value).strip()
                    and str(value).lower() not in ["nan", "none", ""]
                ):
            ...
            if key_parts:
                ...
                df.at[idx, "_dedup_key"] = (
                    f"unique_{idx}"  # Ensure unique key for empty records
                )
        ...
        logger.info(
            f"Deduplication complete: {original_count} → {len(final_df)} records"
        )
