...
class TokenBucket:
    ...
    def __init__(self, rate_limit: int, bucket_size: Optional[int] = None):
        ...
        self.bucket_size = bucket_size or max(rate_limit, 1)
        self.tokens = self.bucket_size
        self.last_update = time.time()
        self.unlimited = rate_limit == 0
    ...
    def update_rate(self, new_rate: int):
        ...
        self.unlimited = new_rate == 0
        if not self.unlimited:
            self.bucket_size = max(new_rate, 1)
            self.tokens = min(self.tokens, self.bucket_size)

class WebhookSender:
    """Handles webhook delivery with intelligent retry and rate limiting"""
    
    def __init__(self, config: WebhookConfig, progress_callback: Optional[Callable] = None):
        """
        ...
        # Failed records for retry
        self.failed_records = []
        
        logger.info(f"Webhook sender initialized for {config.url}")
    ...
    async def send_record_async(self, session: aiohttp.ClientSession, record: Dict[str, Any], record_index: int) -> bool:
        """
        Send single record via webhook asynchronously
            ...
            try:
                ...
                async with session.post(
                    ...
                    if response.status >= 200 and response.status < 300:
                        ...
                        logger.warning(f"Webhook returned status {response.status} for record {record_index}")
                ...
                logger.warning(f"Webhook timeout for record {record_index}, attempt {attempt + 1}")
            except Exception as e:
                logger.warning(f"Webhook error for record {record_index}, attempt {attempt + 1}: {e}")
        ...
        # All retries failed
        self.stats['failed_records'] += 1
        self.failed_records.append({'index': record_index, 'data': record})
        return False
    ...
    async def send_records_batch(self, records: List[Dict[str, Any]], batch_size: int = 10) -> Dict[str, Any]:
        """
        Send records in batches asynchronously
        ...
        self._update_progress("Webhook delivery completed", 100, results)
