...
class Phase2Standardizer:
    ...
    async def map_and_standardize(
        ...
        if progress_callback:
            progress_callback(
                percentage=35,
                message="Extracting headers and sample data for AI mapping...",
                stage="phase2",
            )

        headers = list(merged_df.columns)
        sample_data = (
            merged_df.head(1).to_dict("records")[0] if len(merged_df) > 0 else {}
        )
        ...
        if progress_callback:
            progress_callback(
                percentage=40,
                message="Sending cleaned headers to n8n for AI mapping...",
                stage="n8n_mapping",
            )
        ...
        logger.info(
            f"Creating {len(original_sources)} separate temp files for n8n (one per original file)"
        )

        for source in original_sources:
            # Filter data for this specific source file
            source_df = (
                merged_df[merged_df["Source"] == source]
                if "Source" in merged_df.columns
                else merged_df
            )
            ...
            logger.info(
                f"Created temp file for {source}: {temp_csv_path} ({len(source_df)} total rows)"
            )

        n8n_response = await header_mapper.map_headers(
            file_paths=temp_file_paths, table_type=table_type, session_id=session_id
        )
        ...
        logger.info(
            f"🔍 N8N Response: {json.dumps(n8n_response, indent=2) if n8n_response else 'EMPTY'}"
        )

        if progress_callback:
            progress_callback(
                percentage=50,
                message="Received AI mappings from n8n, applying to data...",
                stage="phase2",
        ...
        logger.info(
            f"Phase 2 complete: {len(standardized_df.columns)} standard columns"
        )
    ...
    def apply_n8n_mappings(
        self, merged_df: pd.DataFrame, n8n_response: Dict, table_type: str
    ) -> pd.DataFrame:
        """
        ...
        logger.info(
            f"🔍 Received n8n_response content: {json.dumps(n8n_response, indent=2) if n8n_response else 'EMPTY'}"
        )
        ...
        if not n8n_response or (isinstance(n8n_response, list) and not n8n_response):
            ...
            raise ValueError(
                "n8n response is empty. Cannot proceed with standardization."
            )
        ...
        if not mapped_files:
            ...
            logger.info(
                f"Available keys in n8n response: {list(n8n_data.keys()) if isinstance(n8n_data, dict) else 'Not a dict'}"
            )
            raise ValueError(
                "n8n response does not contain 'mapped_files'. Cannot proceed."
            )
        ...
        for i, file_mapping in enumerate(mapped_files):
            logger.info(
                f"File {i+1}: {file_mapping.get('filename', 'unknown')} with {len(file_mapping.get('mapped_headers', {}))} mapped headers"
            )
        ...
        for standard_header in standard_headers:
            if standard_header == "Source":
                ...
                mapped_values = self._map_standard_header(
                    merged_df, standard_header, all_mappings
                )
        ...
        unmapped_columns = [
            col for col in merged_df.columns if col not in mapped_original_columns
        ]
        ...
        logger.info(
            f"📦 Preserved {unmapped_count} unmapped columns: {unmapped_columns[:10]}{'...' if len(unmapped_columns) > 10 else ''}"
        )
        ...
        logger.info(
            f"   • Unmapped headers preserved: {self.stats['unmapped_headers']}"
        )
                ...
                for priority in ["primary", "secondary", "tertiary"]:
                    if priority in priority_mappings:
                        ...
                        if source_column not in all_mappings[standard_header][priority]:
                            all_mappings[standard_header][priority].append(
                                source_column
                            )

        return all_mappings

    def _map_standard_header(
        self,
        merged_df: pd.DataFrame,
        standard_header: str,
        ...
        for source_col in mapping_info.get("primary", []):
            if source_col in merged_df.columns:
                ...
                valid_mask = (source_values != "") & (
                    source_values.str.lower() != "nan"
                )

                result_series.loc[mask & valid_mask] = source_values.loc[
                    mask & valid_mask
                ]
                self.stats["primary_mappings"] += (mask & valid_mask).sum()
                logger.debug(
                    f"Primary mapping: {source_col} -> {standard_header} ({(mask & valid_mask).sum()} values)"
                )
        ...
        for source_col in mapping_info.get("secondary", []):
            if source_col in merged_df.columns:
                ...
                logger.debug(
                    f"Secondary mapping: {source_col} -> {standard_header} ({(mask & valid_mask).sum()} values)"
                )
        ...
        for source_col in mapping_info.get("tertiary", []):
            if source_col in merged_df.columns:
                ...
                logger.debug(
                    f"Tertiary mapping: {source_col} -> {standard_header} ({(mask & valid_mask).sum()} values)"
                )

        return result_series

    def _create_empty_standardized_df(
        self, merged_df: pd.DataFrame, standard_headers: List[str]
    ) -> pd.DataFrame:
    ...
    def _consolidate_variant_fields(
        ...
        for target_field, variant_names in consolidation_rules.items():
            ...
            for variant in existing_variants:
                ...
                has_value = (variant_values.str.strip() != "") & (
                    variant_values.str.lower() != "nan"
                )

                if (mask & has_value).any():
                    df.loc[mask & has_value, target_field] = variant_values.loc[
                        mask & has_value
                    ]
                    consolidated_count += (mask & has_value).sum()
                    logger.info(
                        f"   ✓ Merged {(mask & has_value).sum()} values from '{variant}' into '{target_field}'"
                    )
        ...
        logger.info(
            f"📦 Consolidated {consolidated_count} values across variant fields"
        )
        return df
