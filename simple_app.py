            ...
            if processing_mode == 'download':
                ...
                try:
                    ...
                    def send_webhooks_background():
                        ...
                        try:
                            print(f"🚀 Starting background webhook sending for job {job_id}")
...
def run_automatic_cleanup():
    ...
    try:
        ...
        if "error" not in stats:
            ...
            if deleted_count > 0:
                print(f"✅ Cleanup completed: {deleted_count} items deleted, {cleanup.format_size(stats['total_size_freed'])} freed")
            else:
